...
from thzsim2.models import ConstantNK, Drude, DrudeLorentz, Fit, Layer, LorentzOscillator, NKFile, TwoDrude
...
def test_build_sample_supports_two_drude_layers_and_fit_parameters(tmp_path):
    reference_result = _reference_result(tmp_path)
    sample_result = build_sample(
        layers=[
            Layer(
                name="epi",
                thickness_um=Fit(45.0, abs_min=30.0, abs_max=65.0, label="epi_thickness_um"),
                material=TwoDrude(
                    eps_inf=Fit(10.8, abs_min=8.0, abs_max=13.0, label="epi_eps_inf"),
                    plasma_freq1_thz=Fit(0.9, abs_min=0.5, abs_max=1.4, label="epi_plasma1_thz"),
                    gamma1_thz=Fit(0.4, abs_min=0.2, abs_max=0.8, label="epi_gamma1_thz"),
                    plasma_freq2_thz=Fit(2.3, abs_min=1.6, abs_max=3.2, label="epi_plasma2_thz"),
                    gamma2_thz=Fit(3.4, abs_min=2.0, abs_max=5.2, label="epi_gamma2_thz"),
                ),
            )
        ],
        reference=reference_result,
    ...
    layer = sample_result.layers[0]
    assert layer.material_kind == "TwoDrude"
    assert layer.parameters["parameter_entries"][0]["name"] == "eps_inf"
    assert len(sample_result.fit_parameters) == 6
    assert sample_result.resolved_stack["layers"][0]["material"]["kind"] == "TwoDrude"
    assert np.isfinite(layer.n).all()
    assert np.isfinite(layer.k).all()
